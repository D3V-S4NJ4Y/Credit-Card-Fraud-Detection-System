...
if st.sidebar.button("🔑 Login", key="first_login_button"):
    if username and password:
        ...
        if authenticated and user_role == selected_role:
            ...
            st.session_state.login_time = datetime.datetime.now(india).strftime('%Y-%m-%d %H:%M:%S')
            st.session_state.login_attempts = 0
...
if not st.session_state.get('authenticated', False):
    ...
    class AdvancedFeatureEngineering:
        ...
        def create_velocity_features(self, transaction_data, user_history):
            ...
            now = datetime.datetime.now(india).strftime('%Y-%m-%d %H:%M:%S')
    ...
    user_history = [
        {
            'timestamp': datetime.datetime.now(india).strftime('%Y-%m-%d %H:%M:%S') - timedelta(hours=np.random.randint(1, 168)),
    ...
    if enable_realtime:
        ...
        if st.button("🔄 Refresh Real-time Data"):
            ...
            for i in range(10):
                transaction = {
                    'Time': (datetime.datetime.now(india).strftime('%Y-%m-%d %H:%M:%S') - timedelta(seconds=i*5)).strftime('%H:%M:%S'),
...
if st.session_state.get('authenticated', False):
    ...
    with col2:
        ...
        model_health_data = {
            ...
            'Last Updated': [
                datetime.datetime.now(india).strftime('%Y-%m-%d %H:%M:%S') - timedelta(days=1),
                datetime.datetime.now(india).strftime('%Y-%m-%d %H:%M:%S')- timedelta(hours=2),
                datetime.datetime.now(india).strftime('%Y-%m-%d %H:%M:%S') - timedelta(days=10),
                datetime.datetime.now(india).strftime('%Y-%m-%d %H:%M:%S') - timedelta(hours=5)
            ]
        }
