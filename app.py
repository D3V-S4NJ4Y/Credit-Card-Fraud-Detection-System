...
def initialize_users_data():
    ...
    if not os.path.exists(file_path):
        default_users = pd.DataFrame({
            'Username': ['User_user', 'john_analyst', 'sarah_manager', 'mike_admin'],
            'Role': ['User', 'Analyst', 'Manager', 'Admin'],
            'Email': ['User@company.com', 'john@company.com', 'sarah@company.com', 'mike@company.com'],
            'Last_Login': ['Never', '2024-06-01 09:15', '2024-06-01 08:30', '2024-05-31 16:45'],
            'Status': ['Active', 'Active', 'Active', 'Active'],
            'Permissions': ['User', 'Read', 'Read/Write', 'Full'],
            'Department': ['General', 'Risk', 'Compliance', 'IT']
        })
...
# Load users data
users_df = initialize_users_data()
...
# Load users data only after authentication
...
# Role selection
available_roles = ["User", "Analyst", "Manager", "Admin"]  # Added User role
selected_role = st.sidebar.selectbox("👥 Select Role", available_roles)
    ...
    if username and password:
        ...
        if authenticated and user_role == selected_role:
            ...
            if remaining_attempts > 0:
                ...
                current_time = datetime.datetime.now()  # Define current_time here
...
if st.session_state.get('registering', False) and not st.session_state.get('authenticated', False):
    ...
    with st.form("registration_form"):
        ...
        new_department = st.selectbox("Department*", 
            ["General", "Risk", "Compliance", "IT", "Operations"])
...
if st.session_state.get('authenticated', False):
    ...
    if st.sidebar.button("Logout", key="logout_button"):
        ...
        st.session_state.user_role = None
        st.session_state.username = None
        st.session_state.login_time = None  # Clear login time
        st.rerun()
...
if not st.session_state.get('authenticated', False):
    ...
    if st.session_state.get('user_role') == 'Admin':
        ...
        pass
    elif st.session_state.get('user_role') == 'Analyst':
...
with col1:
    ...
    if risk_level == "CRITICAL":
        ...
        if enable_email:
            send_advanced_email_alert("sanjay9852@gmail.com", all_features, risk_data, model_output)
...
if st.session_state.get('user_role') == 'Admin':
    ...
    with tab1:
        ...
        if st.session_state.get('adding_user', False):
            ...
            with st.form("add_user_form"):
                ...
                new_department = st.selectbox("Department*", ["General", "Risk", "Compliance", "IT", "Operations"])
        ...
        if search_user and not users_df.empty:
            ...
            if not filtered_users.empty:
                ...
                display_df = filtered_users.drop('Password', axis=1)
                st.dataframe(display_df, use_container_width=True)
        ...
        if st.session_state.get('show_edit_form', False) and st.session_state.get('selected_user'):
            ...
            with st.form("edit_user_form"):
                ...
                edit_role = st.selectbox(
                    ...
                    index=["User", "Analyst", "Manager", "Admin"].index(user_data['Role'])
                )
                ...
                edit_permissions = st.selectbox(
                    "Permissions",
                    options=available_permissions,
                    index=available_permissions.index(current_permission)
                )
                
                edit_status = st.selectbox(
                    "Status",
                    options=["Active", "Inactive"],
                    index=["Active", "Inactive"].index(user_data['Status'])
                )
    ...
    with tab2:
        ...
        log_data = {
            ...
            'User': np.random.choice(['analyst', 'manager', 'admin'], 20),
...
# Add this in the risk analysis section where alerts are triggered
if risk_data['risk_level'] in ['CRITICAL', 'HIGH']:
    ...
    try:
        ...
        transaction_details = {
            'amount': f"${amount:,.2f}",
            'timestamp': datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'location': f"Distance: {all_features['location_distance']:.2f} km",
            'risk_score': f"{risk_data['total_risk_score']:.2%}"
        }
